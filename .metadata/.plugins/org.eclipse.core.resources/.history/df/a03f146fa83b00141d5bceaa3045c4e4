...
public class Hybrid_Framework_WebDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		String eXPath;
		String xlPath, xlTC_res, xlTS_res;
		String xlTC_res_temp, xlTS_res_temp;
		String vTS_res, vTC_res, vTD_res, vTS_error ;
		String vT_flag;
		
		// Read the Test Steps XL sheet.
		xlPath = "C:/Selenium/Aug18/KDF/Automation Plan 1.1.xls";
		xlTC_res= "C:/Selenium/Aug18/KDF/Results_TC";
		xlTS_res= "C:/Selenium/Aug18/KDF/Results_TS";
		xlTSRead(xlPath);
		xlTCRead(xlPath);
		xlTDRead(xlPath);
		...
		for (int k=1;k<xTDRows; k++){ // Go to each Test data row
			...
			// Then load the variables
					...
					if (xTCdata[j][4].equals("Y")) { //		If y then go to each row in TS Sheet
						...
						for (int i = 1; i <xTSRows; i++){
							if (xTCdata[j][1].equals(xTSdata[i][0])){	// and see if TCID's match
								...
								vT_flag = "Pass";
								try {
									vT_flag = keyword_executor();
									if (vT_flag.equals("Fail")) {
										vTS_res = "Fail";
								...
								xTSdata[i][8] = vTS_res; //Take the TS result back to the XL array 
								xTSdata[i][9] = vTS_error;
								if (vTS_res.equals("Fail")) { // Set TC to fail if any Test Step fails.
									vTC_res = "Fail";
								}
							}
						}
				...
				xlTC_res_temp = xlTC_res + xTDdata[k][0] + ".xls"; 
				xlTS_res_temp = xlTS_res + xTDdata[k][0] + ".xls";
				xlwrite_tc(xlTC_res_temp, xTCdata);
				xlwrite_ts(xlTS_res_temp, xTSdata);
			}
		}
	...
	public static void xlwrite_tc(String xlPath, String[][] xldata) throws Exception {
		System.out.println("Inside XL Write");
    	File outFile = new File(xlPath);
        HSSFWorkbook wb = new HSSFWorkbook();
        HSSFSheet osheet = wb.createSheet("TestCases_Res");
    	for (int myrow = 0; myrow < xTCRows; myrow++) {
            HSSFRow row = osheet.createRow(myrow);
	        for (int mycol = 0; mycol < xTCCols; mycol++) {
	...
	public static void xlwrite_ts(String xlPath, String[][] xldata) throws Exception {
		System.out.println("Inside XL Write");
    	File outFile = new File(xlPath);
        HSSFWorkbook wb = new HSSFWorkbook();
        HSSFSheet osheet = wb.createSheet("TestSteps_Res");
    	for (int myrow = 0; myrow < xTSRows; myrow++) {
            HSSFRow row = osheet.createRow(myrow);
	        for (int mycol = 0; mycol < xTSCols; mycol++) {
	        	HSSFCell cell = row.createCell(mycol);
	        	cell.setCellType(HSSFCell.CELL_TYPE_STRING);
	        	cell.setCellValue(xldata[myrow][mycol]);
	        }
